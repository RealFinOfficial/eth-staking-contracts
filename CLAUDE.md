...
- **`LPStakingVault.sol`** — custody only, behind a **UUPS proxy** (`deploy/LPProxy.sol`).
  ...
  **operator** (a multisig, no delay): `setTwapParams`, `rescuePosition` (the NFT goes to the
  operator), `setGuardian`, and those two pauses as well, as the cold fallback for a lost
  guardian key. Ownership is two-step and `renounceOwnership` reverts. `stakeFor` takes two
  routes in: the single `zapper` slot, and the `setStakeOperator` allowlist a second deposit
  route uses without displacing the zapper
...
test/                — Hardhat test files (Mocha + Chai). 783 tests, 0 pending
  ...
  lp-staking/
    LPStakingVault.test.js      — 105 tests, incl. the upgrade, timelock and stake-operator paths
...
test/forge/          — Foundry tier. 498 tests in 26 suites: 101 fork, 358 unit,
...
Re-measured 2026-09-15 — branch coverage is 100% on all seven files, so every branch floor is
